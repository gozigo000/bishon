...
class KipoInspector {
    ...
    private startPreInspection() {
        ...
        if (/>\s+</.test(this.roughXml)) this.report.record({ kind: '개발', msg: '<태그> 사이에 공백이 있습니다.' });
        ...
        if (!headTags) return this.report.record({ kind: '개발', msg: '<KIPO> 등 머리 태그가 없습니다.' });
        this.headTags = headTags.replaceAll('><', '>\n<');

        const tailTags = this.roughXml.match(/<\/PatentCAFDOC><\/KIPO>/)?.[0];
        if (!tailTags) return this.report.record({ kind: '개발', msg: '</KIPO> 등 꼬리 태그가 없습니다.' });
        ...
        if (/<p num="\w*">\s*<\/p>/.test(this.roughXml)) {
            ...
            this.report.record({ kind: '개발', pos: `문단 ${pNums.join(', ')}`, msg: `공백이 있는 <p> 태그가 존재합니다.` });
        };
    ...
    private startMainInspection() {
        ...
        if (!disc.isInSpec) return this.report.record({ kind: '에러', pos: '[발명의 설명]', msg: '목차가 없습니다.' });
    ...
    private inspect_invenTitle(disc: KXmlPouch): KXml {
        ...
        if (!title.isInSpec) return this.report.record({ kind: '에러', pos: '[발명의 명칭]', msg: '필수 목차가 없습니다.' });

        let ttl = title.getInnerXml().
            replaceAll(/\s+/g, ' '). // 줄바꿈 제거
            replaceAll(/<.*?>/g, ''); // 태그 제거

        if (!/{/.test(ttl)) return this.report.record({ kind: '에러', pos: '[발명의 명칭]', msg: '발명의 명칭에 여는 중괄호({)가 없습니다.' });
        if (!/}/.test(ttl)) return this.report.record({ kind: '에러', pos: '[발명의 명칭]', msg: '발명의 명칭에 닫는 중괄호(})가 없습니다.' });

        const match = ttl.match(/(.+)\{(.+?)\}/);
        if (!match) return this.report.record({ kind: '에러', pos: '[발명의 명칭]', msg: '발명의 명칭에 한글 또는 영문 명칭이 없습니다.' });
        ...
        const enTitle = match[2].
            ...
            replaceAll(/\s+/g, ' ').trim();
        if (!/[가-힣]+/.test(krTitle)) return this.report.record({ kind: '에러', pos: '[발명의 명칭]', msg: '발명의 명칭에 한글 명칭이 없습니다.' });
        if (!/[a-zA-Z]+/.test(enTitle)) return this.report.record({ kind: '에러', pos: '[발명의 명칭]', msg: '발명의 명칭에 영문 명칭이 없습니다.' });
        if (krTitle.length > 270) return this.report.record({ kind: '에러', pos: '[발명의 명칭]', msg: '한글 명칭이 270자를 초과합니다.' });
        if (enTitle.length > 540) return this.report.record({ kind: '에러', pos: '[발명의 명칭]', msg: '영문 명칭이 540자를 초과합니다.' });
    ...
    private inspect_table(tables: string): string {
        ...
        if (img) {
            if (img.length > 1) {
                this.report.record({ kind: '에러', pos: `[표 ${tNum}]`, msg: `표에 복수의 이미지가 있습니다.`})
            };
        ...
        if (tbl) {
            if (tbl.length > 1) {
                this.report.record({ kind: '에러', pos: `[표 ${tNum}]`, msg: `복수의 표가 있습니다.`})
            };
        ...
        return this.report.record({ kind: '에러', pos: `[표 ${tNum}]`, msg: `표에 내용이 없습니다.`})
    }
    ...
    private inspect_math(maths: string): string {
        ...
        if (!img) {
            return this.report.record({ kind: '에러', pos: `[수학식 ${eNum}]`, msg: `수학식 이미지가 없습니다.`})
        };
        if (img.length > 1) {
            return this.report.record({ kind: '에러', pos: `[수학식 ${eNum}]`, msg: `복수의 수학식 이미지가 있습니다.`})
        };
    ...
    private inspect_claims(spec: string): string {
        ...
        for (const [i, clm] of claim.getInnerXmls().entries()) {
            ...
            if (/^\s*삭\s*제[\.\,\s]*$/.test(clm)) {
                this.report.record({ kind: '에러', pos: `[청구항 ${cNum}]`, msg: `청구항을 삭제하려면 해당 청구항 자체를 목차에서 삭제하세요: [청구항 ${cNum}] -> 전부 삭제`});
            }
            // TODO: 화학식/반응식 검사 추가
            if (/<(tables|maths) .*?>/.test(clm) ||
                /\[(표|수학식|화학식|반응식) ?\w+?\]/.test(clm)
            ) {
                this.report.record({ kind: '에러', pos: `[청구항 ${cNum}]`, msg: `청구항에는 [표], [수학식], [화학식], [반응식]을 삽입할 수 없습니다.`});
            }
            if (/<img .*?>/.test(clm)) {
    ...
    private inspect_abstract(spec: string): string {
        ...
        if (!summary.isInSpec) {
            this.report.record({ kind: '에러', pos: '[요약서]', msg: `요약서에 [요약]이 없습니다.`});
        }

        let absFig = new KXmlPouch('<대표도>', '<//대표도>', abstract.getInnerXml());
        if (this.counter.fig.cnt > 0) {
            if (!absFig.isInSpec) {
                this.report.record({ kind: '에러', pos: '[요약서]', msg: `[도면]을 작성한 경우 [요약서]에 [대표도]를 기재해야 합니다.`});
            } 
            else {
                const regex = /<figref num="(\w*)"/;
                const xml = absFig.getInnerXml();
                if (!regex.test(xml)) {
                    this.report.record({ kind: '에러', pos: '[대표도]', msg: `[대표도]에 도면 번호를 기재해야 합니다.`});
                }
                const absFigNum = xml.match(regex)![1];
                if (!this.counter.fig.nums.includes(absFigNum)) {
                    this.report.record({ kind: '에러', pos: '[대표도]', msg: `[대표도]에 기재된 도면 번호(도 ${absFigNum})가 [도면]에 없는 번호입니다.`});
                }
            }
        }
    ...
    private inspect_drawings(spec: string): string {
        ...
        for (const [i, fig] of figs.getInnerXmls().entries()) {
            ...
            if (!imgs) {
                this.report.record({ kind: '에러', pos: `[도 ${fNum}]`, msg: `도면에 이미지가 없습니다.`});
                continue;
            }
            if (imgs.length > 1) {
                this.report.record({ kind: '에러', pos: `[도 ${fNum}]`, msg: `하나의 도면에 복수의 이미지가 있습니다.`});
                continue;
            }
    ...
    private inspect_img(img: string): string {
        ...
        if (!imgInfo) return this.report.record({ kind: '개발', pos: img, msg: `이미지 태그 형식이 잘못되었습니다.`});

        if (!imgInfo[1].match(/^i\d{4}$/)) return this.report.record({ kind: '개발', pos: img, msg: `이미지 번호가 올바르지 않습니다.`});
        if (Number(imgInfo[2]) > 222) return this.report.record({ kind: '개발', pos: img, msg: `이미지 높이가 222mm를 초과합니다.`});
        if (Number(imgInfo[3]) > 165) return this.report.record({ kind: '개발', pos: img, msg: `이미지 너비가 165mm를 초과합니다.`});
        if (!imgInfo[4].match(/^\d{5}$/)) return this.report.record({ kind: '개발', pos: img, msg: `이미지 파일명이 올바르지 않습니다.`});
        if (!imgInfo[5].match(/^(jpe?g|tiff?)$/)) return this.report.record({ kind: '개발', pos: img, msg: `이미지 파일 확장자가 올바르지 않습니다.`});
    ...
    private inspect_numbering(title: string, nums: string[]) {
        ...
        for (const num of nums) {
            ...
            if (!/^\d{1,3}[a-z]?$/.test(num)) {
                this.report.record({ kind: '에러', pos: numTitle, msg: `${title} 번호는 숫자(최대 3자리) + 알파벳 소문자(1자)만 가능합니다.`});
            }
            // 중복 검사
            if (numSet.has(num)) {
                this.report.record({ kind: '에러', pos: numTitle, msg: `${title} 번호가 중복 사용되었습니다.`});
            }
            numSet.add(num);
            ...
            if (digit === 1 + prevDigit && (alpha === '' || alpha === 'a')) {
                ...
                this.report.record({ kind: '에러', pos: `${prevNumTitle}, ${numTitle}`, msg: `숫자가 같은 ${title} 번호는 알파벳을 순차적으로 부여해야 합니다.`});
                break;
            }
            else if (digit !== 1 + prevDigit && prevDigit > 0) {
                // 숫자 부분이 1씩 증가하지 않는 경우 (예: 5/5a -> 4/4a, 5/5a -> 7/7a)
                this.report.record({ kind: '에러', pos: `${prevNumTitle}, ${numTitle}`, msg: `${title} 번호는 순차적으로 증가(1, 2, 3, ...)해야 합니다.`});
                break;
            }
            else if ((digit !== 1 || num !== '1a') && prevDigit === 0) {
                // 첫번째 번호가 '1' 또는 '1a'가 아닌 경우 (예: 2, 1b)
                this.report.record({ kind: '에러', pos: numTitle, msg: `${title} 번호는 '1' 또는 '1a'로 시작해야 합니다.`});
                break;
            }
            else {
                // 그 외의 경우 - HACK: 여기에 걸리는 경우가 있는지 체크
                this.report.record({ kind: '에러', pos: numTitle, msg: `${title} 번호에 오류가 있는 것 같습니다.`});
                break;
            }
            prevNum = num;
            prevDigit = digit;
    ...
    private inspect_claimNumbering(nums: string[]) {
        ...
        for (const num of nums) {
            // 숫자만 사용 가능
            if (!/^\d+$/.test(num)) {
                this.report.record({ kind: '에러', pos: `[청구항 ${num}]`, msg: `청구항 번호는 숫자만 사용할 수 있습니다.`});
            }
            // 중복 검사
            if (numSet.has(num)) {
                this.report.record({ kind: '에러', pos: `[청구항 ${num}]`, msg: `청구항 번호가 중복 사용되었습니다.`});
            }
            // 1씩 증가 검사
            if (Number(num) !== prevNum + 1) {
                this.report.record({ kind: '에러', pos: `[청구항 ${prevNum}], [청구항 ${num}]`, msg: `청구항 번호를 순차적으로(1,2,3,...) 부여해야 합니다.`});
            }
            prevNum += 1;
            numSet.add(num);
        }
    ...
    private inspect_mustHaveTitles({
        ...
        if (!disc.isInSpec) this.report.record({ kind: '에러', pos: '[발명의 설명]', msg: '필수 목차가 없습니다.'});
        if (!title) this.report.record({ kind: '에러', pos: '[발명의 명칭]', msg: '필수 목차가 없습니다.'});
        if (!field.isInSpec) this.report.record({ kind: '에러', pos: '[기술분야]', msg: '필수 목차가 없습니다.'});
        if (!background.isInSpec) this.report.record({ kind: '에러', pos: '[배경기술]', msg: '필수 목차가 없습니다.'});
        if (!invenSummary) this.report.record({ kind: '에러', pos: '[발명의 내용]', msg: '필수 목차가 없습니다.'});
        if (!drawings && this.counter.fig.cnt > 0) {
            if (!briefDrawings.isInSpec) this.report.record({ kind: '에러', pos: '[도면의 간단한 설명]', msg: '도면을 작성한 경우 도면의 간단한 설명을 작성해야 합니다.'});
        }
        if (!embodiments) this.report.record({ kind: '에러', pos: '[발명을 실시하기 위한 구체적인 내용]', msg: '필수 목차가 없습니다.'});
        if (!claims) this.report.record({ kind: '에러', pos: '[청구범위]', msg: '필수 목차가 없습니다.'});
        if (!abstract) this.report.record({ kind: '에러', pos: '[요약서]', msg: '필수 목차가 없습니다.'});
